...
class Response {
	...
	public function __construct( $gateway ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		\WC_Gateway_Nabil_Bank::log( 'Checking Response...' );
		...
		$endpoint = 'yes' === $test_mode ? 'https://stagesys.nabilbank.com/api/EPayment/CheckStatus' : 'https://sys.nabilbank.com/api/EPayment/CheckStatus';
		...
		if ( is_wp_error( $response ) ) {
			\WC_Gateway_Nabil_Bank::log( 'Error response: ' . $response->get_error_message() );
			return;
		}
		...
		if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 && strstr( strtoupper( $response['body'] ), 'SUCCESS' ) ) {
			\WC_Gateway_Nabil_Bank::log( 'Received valid response from Nabil Bank' );
		}
	...
	public function get_order_by_invoice_id( $invoice_id, $gateway ) {
		...
		if ( ! $order ) {
			...
			// Nothing was found.
			\WC_Gateway_Nabil_Bank::log( 'Order not found.', 'error' );
		}
